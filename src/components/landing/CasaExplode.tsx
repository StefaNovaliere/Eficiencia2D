 ...
 *     de polvo neón) hacia su plancha de corte y, al aterrizar, se convierte en un panel
 *     vector 2D plano. Pisos/losas → P1; paredes + carpinterías → P2. Paneles anidados
 *     con separación tipo 3mm, proporciones reales de cada pieza.
 *
 * El layout se escala para entrar completo en pantallas de laptop (ver CSS: el svg se
 * limita por ancho y por alto). Colores crema/tinta del SVG sobrescritos por CSS.
 */
...
interface PieceRef {
  ...
  bw: number;
  bh: number;
  // destino (panel anidado) + parámetros de vuelo
  pCx: number;
  pCy: number;
  k: number;
  ctrlX: number;
...
const P1: Plancha = { x: 330, y: 470, w: 500, h: 820, label: "P1 · pisos" };
const P2: Plancha = { x: 880, y: 250, w: 640, h: 1080, label: "P2 · muros" };

const PAD = 34; // margen interno de la plancha
...
/**
 * Empaquetado por estantes (shelf packing): ordena por alto desc y baja la escala global
 * hasta que entra en el área útil. Conserva la proporción real de cada pieza, así las
 * paredes quedan grandes, las ventanas chicas y la baranda larga y fina (como una plancha
 * de corte real). Devuelve la escala k y la posición de cada panel en coords internas.
 */
function packShelf(
...
export default function CasaExplode() {
  ...
  useEffect(() => {
    ...
    // Empaquetar cada grupo y crear su panel 2D + pool de polvo.
    const assignGroup = (list: PieceRef[], pl: Plancha) => {
      ...
      const { k, placed } = packShelf(
        list.map((ref) => ({ w: ref.bw, h: ref.bh, ref })),
        innerW,
        innerH,
      );
      for (const pp of placed) {
        const px = innerX + pp.x;
        const py = innerY + pp.y;
        pp.ref.k = k;
        pp.ref.pCx = px + pp.w / 2;
        pp.ref.pCy = py + pp.h / 2;
        ...
        outer.setAttribute("width", String(pp.w));
        outer.setAttribute("height", String(pp.h));
        g.appendChild(outer);
        const off = Math.min(pp.w, pp.h) * 0.12;
        if (off > 3) {
          ...
          inset.setAttribute("width", String(pp.w - off * 2));
          inset.setAttribute("height", String(pp.h - off * 2));
          g.appendChild(inset);
        }
    ...
    const ranked = [...pieces].sort((a, b) => a.pCy - b.pCy || a.pCx - b.pCx);
    ranked.forEach((g, i) => {
      g.startT = (i / Math.max(1, ranked.length - 1)) * STAGGER_SPAN;
      const p0x = g.cx + g.dx;
      const p0y = g.cy + g.dy;
      const mx = (p0x + g.pCx) / 2;
      const my = (p0y + g.pCy) / 2;
      const dx = g.pCx - p0x;
      const dy = g.pCy - p0y;
      const dist = Math.hypot(dx, dy) || 1;
      const side = i % 2 === 0 ? 1 : -1;
      // control perpendicular a la trayectoria + leve elevación → arco
    ...
    const render = (p: number) => {
      ...
      for (const g of pieces) {
        if (pNest <= 0) {
          ...
          g.panelEl.style.opacity = "0";
          for (const c of g.dust) c.style.opacity = "0";
          g.depth = g.d0 + g.dr * eExp;
          continue;
        }
        // Fase vuelo/nest: arco + giro + crossfade a panel 2D.
        const fp = clamp01((pNest - g.startT) / FLIGHT_DUR);
        const ef = ease(fp);
        const p0x = g.cx + g.dx;
        const p0y = g.cy + g.dy;
        const bx = bezier(ef, p0x, g.ctrlX, g.pCx);
        const by = bezier(ef, p0y, g.ctrlY, g.pCy);
        const s = lerp(1, g.k, ef);
        ...
        g.panelEl.style.opacity = String(panelOp);
        ...
        for (let j = 0; j < g.dust.length; j++) {
          ...
          c.setAttribute("cx", String(bezier(tj, p0x, g.ctrlX, g.pCx) + (-dyy / dl) * jit));
          c.setAttribute("cy", String(bezier(tj, p0y, g.ctrlY, g.pCy) + (dxx / dl) * jit));
